...
describe('star-rating component', () => {
    ...
    describe('dom events', () => {
        ...
        it('should add the given number of stars to the page ', () => {
            ...
            // expect there to be 10 polygons, because we have 2 polygons for each star (3 if you set glow color) to account for border and glow
            // so, 5 stars is 10 polygons
            expect(stars.length).toEqual(10);
        });


        it('should emit current-rating event on mousemove', async () => {
            const wrapper = mount(StarRating)
            let star = wrapper.find('polygon')
            await star.trigger('mousemove')

            expect(wrapper.emitted()).toHaveProperty('current-rating')
        });


        it('should emit the rating-selected event on click', async () => {
            const wrapper = mount(StarRating)
            let star = wrapper.find('polygon')
            await star.trigger('click')

            expect(wrapper.emitted()).toHaveProperty('rating-selected')
        });
